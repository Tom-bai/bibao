            ...
            <div class="form-wrap">
                ...
                <el-form :model="Form" :rules="rules" ref="Form" class="demo-ruleForm">
                    ...
                    <el-form-item prop="phone" v-if="defaultIndex==1">
                        <el-input v-model="Form.phone" :placeholder="$t('MOBILE_LABEL')">
                            <i slot="prefix" class="el-input__icon icon-user"></i>
                        </el-input>
                    </el-form-item>
                    <!-- 邮箱找回 -->
                    <el-form-item prop="email" v-if="defaultIndex==2">
                        <el-input v-model="Form.email" :placeholder="$t('EMAIL_LBAEL')">
                            <i slot="prefix" class="el-input__icon icon-user"></i>
                        </el-input>
                    </el-form-item>
                    ...
                    <el-form-item prop="pwd2">
                        <el-input v-model="Form.pwd2" @keyup.enter.native="submitForm('Form')" type="password" :placeholder="$t('CHECK_PASS')">
                            <i slot="prefix" class="el-input__icon icon-pass"></i>
                        </el-input>
                    </el-form-item>
                    <el-form-item>
                        <el-button type="primary" @click="submitForm('Form')" :loading="isAble" :disabled="isAble" class="flex-btn">{{$t('FIND_PWD')}}</el-button>
                    </el-form-item>
                    <p class="box-flex form-text center">
                        <span @click="$router.push('/signin')">{{'LOGIN_TITLE'|t}}>></span>
                    </p>
                </el-form>
            </div>
        </div>
...
<script>
    ...
    export default {
        data() {
            ...
            let _checkExist = (rule, value, callback) => {
                ...
                Login.checkMember(params).then(res => {
                    if (!res.oneData) callback(new Error("该用户名未注册"));
                    else callback();
                });
            };
        ...
        methods: {
            // 登陆验证码图片
            getYZM() {
                this.YZMsrc = Login.fgCaptcha + "?" + Math.random()+'&bgColor=' +  this.captchaBgColor;
            },
