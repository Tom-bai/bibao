var GLOBAL_VAR = {
    ...
    button_down: false,
    init: false,
    url: klineUrl,// ws://192.168.50.126:8099/websocket/Kline/v1
};
...
ChartManager.prototype.__construct = function () {
    ...
    this._language = GLOBAL_VAR.defLang;
...
var ws = null;
//改寫為js方法，使用者要自己寫後台請求，如websocket
var RequestData = function (showLoading) {
    ...
    var type = queryString(canshu.requestParam, "type");
    var needTickers = queryString(canshu.requestParam, "needTickers");
    var symbol = GetQueryString('symbol');
    if (symbol == 'undefined-undefined') {
        return false;
    }
    //心跳检测
    var heartCheck = {
    ...
    let parmas = {
        symbol: symbol,
        needTickers: needTickers,
        type: type,
        size: size
    }
    // var data = { "des": "注释", "isSuc": true, "datas": { "USDCNY": 1000, "contractUnit": "BTC", "data": [[1499875200000, 165000080, 167000000, 1600000530, 167000000, 12000000.6415], [1499875200000, 16680, 16200, 16730, 16900, 9.6415], [1499875200000, 16552, 16980, 16130, 16400, 17.6415], [1499875200000, 16580, 16700, 16530, 16700, 15.6415], [1499875200000, 16580, 16700, 16530, 16700, 36.6415], [1499875200000, 16580, 16700, 16530, 16700, 85.6415], [1499918040000, 16630, 16784, 16582, 16710, 42.6415], [1499918280000, 16580, 16700, 16530, 16700, 32.6415], [1499918400000, 16580, 16700, 16530, 16700, 22.6415], [1499918760000, 16580, 16700, 16530, 16700, 17.6415], [1499919000000, 16548, 16860, 16430, 16800, 19.6415]], "marketName": "币柜网", "moneyType": "CNY", "symbol": "btc38btccny", "url": "官网地址", "topTickers": [] } }
    if (!ws) {
        ws = new WebSocket(GLOBAL_VAR.url);
        ws.onopen = function (evt) {
            heartCheck.reset().start();
            // console.log("Connection open ...");
            // GLOBAL_VAR.requestParam = setHttpRequestParam(1, GLOBAL_VAR.time_type, null, 100000000000);
    ...
    ws.onmessage = function (evt) {
        if (evt.data == 'fine') {
            ...
            GLOBAL_VAR.KLineData = data.data.datas;
...
function readCookie() {
    ...
    chart_switch_language( GLOBAL_VAR.defLang || b.language )
}
var main = function () {
